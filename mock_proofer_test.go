...
func (_m *MockProofer) HashEx(ctx context.Context, block Block) (string, error) {
	...
	if rf, ok := ret.Get(0).(func(context.Context, Block) string); ok {
		r0 = rf(ctx, block)
	} else {
		r0 = ret.Get(0).(string)
	}

	if rf, ok := ret.Get(1).(func(context.Context, Block) error); ok {
		r1 = rf(ctx, block)
	} else {
		r1 = ret.Error(1)
	}
